...
import { create, findOne, update } from "../../lib/backend";
...
const Index = () => {
  ...
  async function salvaDadosNoBanco() {
    try {
      if (localStorage.getItem("idServidor")) {
        await update(idServidor, serializaDados());
      }
    } catch (err) {
      throw err;
    }
  ...
  function setDados(dados) {
    ...
    setSaveState({
      ...
      jogador1: dados.jogador1 || {
        ...
        decks: [
          {
            code: "CECAMAIABEFRMHJGE4AQEBQWAIBAACIKAEBQABQEAEBAABYBAMAA4AQBAAKRUAICAY7AA",
            regions: ["bilgewater", "demacia"],
            champions: [
              { nome: "Lucian", qtd: 3 },
              { nome: "MissFortune", qtd: 3 },
            ],
          },
          {
      ...
      jogador2: dados.jogador2 || {
        ...
        time: {
          nome: "WNX",
          url_logo:
            "https://media.discordapp.net/attachments/703593969820631050/739497322580541540/WNX.png",
        },
        decks: [
          {
            code: "CEBAIAYGAQDQQDYHAMER2IZNGM2DOPACAEBQMCIBAMESSAQBAIDAKAQDBEKFK",
            regions: ["bilgewater", "targon"],
            champions: [
              { nome: "Soraka", qtd: 3 },
              { nome: "TahmKench", qtd: 3 },
            ],
          },
          {
      ...
      bans: [
        dados.bans1 || [true, false, false],
        dados.bans2 || [false, false, true],
      ],
      vitorias: [
        dados.vitorias1 || [false, true, false],
        dados.vitorias2 || [false, false, false],
      ],
      atuais: [
        dados.atuais1 || [false, false, true],
        dados.atuais2 || [false, true, true],
      ],
  ...
  async function criaIdServidor() {
    try {
      const response = await create(serializaDados());
      setDados(response);
    } catch (err) {
      console.log(err);
      return false;
  ...
  async function carregaDadosDoBanco() {
    try {
      if (localStorage.getItem("idServidor")) {
        ...
        const response = await findOne(idQuery);
